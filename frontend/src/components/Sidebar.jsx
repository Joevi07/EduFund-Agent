...
export default function Sidebar({ activeTab, setActiveTab, profile, user, onLogout, collapsed, setCollapsed }) {
  ...
  if (user?.role === "admin") mainNav.push({ id: "admin", label: "Admin Console", icon: Settings });

  return (
    <aside className="app-sidebar" style={{ width: collapsed ? "70px" : "260px" }}>
      <div>
        {/* Brand Header (SufeeAdmin Style) */}
        ...
        <nav style={{ display: "flex", flexDirection: "column", gap: "0.2rem", padding: "0 0.6rem" }}>
          {agentNav.map(item => {
            const Icon = item.icon;
            const isActive = activeTab === item.id;
            return (
      ...
      {!collapsed && (
        <div style={{
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <div style={{ fontSize: "0.7rem", color: "#8796a6", display:"flex", justifyContent:"space-between" }}>{user?.role === "admin" ? "Administrator" : "Student"}<button onClick={onLogout} style={{background:"none",border:0,color:"#20a8d8",cursor:"pointer",fontSize:"0.7rem"}}>Sign out</button></div>
          </div>
        </div>
      )}
    </aside>
